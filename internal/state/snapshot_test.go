...
func TestSnapshotIdentityOrderAndPrivacy(t *testing.T) {
	...
	first := s.Snapshot()
	if len(first.Sources) != 2 || first.Sources[0].ID == first.Sources[1].ID { t.Fatal("sources were merged by homepage") }
	encoded, _ := json.Marshal(first)
	for _, secret := range []string{"private", "user:secret", "auth="} { if strings.Contains(string(encoded), secret) { t.Fatal("snapshot exposes configured credentials") } }
	s.ReplaceConfig(config.Config{Values: []string{b, a}})
	if s.Snapshot().Sources[1].ID != first.Sources[0].ID { t.Fatal("source ID depends on order") }
	s.ReplaceConfig(config.Config{})
	if len(s.Snapshot().Sources) != 0 { t.Fatal("deleted sources remain in snapshot") }
}

func TestFetchStateAndContentTimestamps(t *testing.T) {
	const url = "https://example.com/rss"
	s := New(config.Config{Values: []string{url}})
	if s.Snapshot().Sources[0].Status != "loading" { t.Fatal("missing source should be visible as loading") }
	...
	if first.LastSuccessAt == second.LastSuccessAt || first.ContentChangedAt != second.ContentChangedAt { t.Fatal("success time and content change time are conflated") }
	...
	if len(s.Snapshot().Sources[0].Items) != 2 { t.Fatal("later item lost behind pinned first item") }
	...
	if stale.Status != "stale" || len(stale.Items) != 2 || strings.Contains(stale.Message, "secret") { t.Fatal("failure should preserve cache and redact errors") }
}
...
func TestRemovedInFlightFetchCannotResurrectSource(t *testing.T) {
	...
	if _, ok := s.BeginFetch(url, time.Now()); ok { t.Fatal("overlapping fetch accepted") }
	s.ReplaceConfig(config.Config{})
	s.ReplaceConfig(config.Config{Values: []string{url}})
	if s.CompleteFetch(url, ticket, domain.Feed{Title: "obsolete"}, time.Now(), nil) { t.Fatal("obsolete fetch accepted after removal/re-add") }
}

func TestSnapshotIsDetached(t *testing.T) {
	s := New(config.Config{Values: []string{"a"}})
	s.SetFeed("a", domain.Feed{Title: "A", Items: []domain.Item{{Title: "original"}}})
	x := s.Snapshot(); x.Sources[0].Items[0].Title = "mutated"
	if s.Snapshot().Sources[0].Items[0].Title != "original" { t.Fatal("snapshot aliases state") }
}
