...
func (s *State) ReplaceConfig(conf config.Config) {
	...
	for raw := range s.feeds {
		if !s.configured(raw) {
			delete(s.feeds, raw)
		}
	}
	for raw := range s.status {
		if !s.configured(raw) {
			delete(s.status, raw)
		}
	}
	for raw := range s.inflight {
		if !s.configured(raw) {
...
func (s *State) BeginFetch(raw string, at time.Time) (uint64, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if !s.configured(raw) || s.inflight[raw] != 0 {
		return 0, false
	}
	s.nextTicket++
	s.inflight[raw] = s.nextTicket
	st := s.status[raw]
	st.attempt = at.UTC().Format(time.RFC3339Nano)
	s.status[raw] = st
...
func (s *State) CompleteFetch(raw string, ticket uint64, feed domain.Feed, at time.Time, fetchErr error) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	if ticket == 0 || s.inflight[raw] != ticket || !s.configured(raw) {
		return false
	}
	delete(s.inflight, raw)
	st := s.status[raw]
	...
	if fetchErr == nil {
		...
		old, ok := s.feeds[raw]
		if !ok || old.Title != feed.Title || old.Link != feed.Link || !reflect.DeepEqual(old.Items, feed.Items) {
			st.changed = stamp
		}
		st.success = stamp
		s.feeds[raw] = cloneFeed(feed)
...
func (s *State) Feeds() []domain.Feed {
	...
	for _, raw := range s.config.Values {
		if f, ok := s.feeds[raw]; ok {
			feeds = append(feeds, cloneFeed(f))
		}
	}
	return feeds
}
...
func (s *State) Snapshot() domain.Snapshot {
	...
	for _, raw := range s.config.Values {
		...
		if st.failed {
			view.Status = "error"
			view.Message = "获取失败，将在下一轮自动重试"
			if cached {
				view.Status = "stale"
				view.Message = "同步失败，正在显示上次成功的内容"
			}
		}
...
func safeItemLink(raw string) string {
	...
	for key := range q {
		...
		case "auth", "token", "access_token", "password", "api_key", "apikey", "signature", "sign":
			q.Del(key)
		}
	}
...
func cloneFeed(feed domain.Feed) domain.Feed {
	...
	if feed.Custom != nil {
		cloned.Custom = make(map[string]string, len(feed.Custom))
