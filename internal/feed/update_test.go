...
func TestUpdateFeedChangesAfterPinnedItemAndKeepsFailureCache(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if revision.Load() == 2 { http.Error(w, "private upstream error", 503); return }
	...
	a, err := archive.Open(filepath.Join(t.TempDir(), "archive.txt")); if err != nil { t.Fatal(err) }
	UpdateFeed(context.Background(), s, a, server.URL, "ignored")
	revision.Store(1)
	UpdateFeed(context.Background(), s, a, server.URL, "ignored")
	if got := s.Snapshot().Sources[0].Items[1].Title; got != "Revision 1" { t.Fatalf("stale later item: %s", got) }
	revision.Store(2)
	UpdateFeed(context.Background(), s, a, server.URL, "ignored")
	if got := s.Snapshot().Sources[0]; got.Status != "stale" || len(got.Items) != 2 { t.Fatal("cache missing on fetch failure") }
}
