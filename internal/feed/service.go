...
func UpdateFeeds(ctx context.Context, state *appstate.State, archiveStore *archive.Store) {
	for {
		...
		conf := state.Config()
		for _, raw := range conf.Values {
			go UpdateFeed(ctx, state, archiveStore, raw, "")
		}
...
func UpdateFeed(ctx context.Context, state *appstate.State, archiveStore *archive.Store, feedURL, _ string) {
	...
	if err != nil || result == nil {
		if err == nil {
			err = fmt.Errorf("empty parser result")
		}
		state.CompleteFetch(feedURL, ticket, domain.Feed{}, time.Now(), err)
		if ctx.Err() == nil {
			log.Printf("feed fetch failed (%T); retrying next cycle", err)
		}
		return
	}
	...
	if !state.CompleteFetch(feedURL, ticket, customFeed, time.Now(), nil) {
		return
	}
	for _, item := range result.Items {
...
func WatchConfigFileChanges(ctx context.Context, filePath string, state *appstate.State, archiveStore *archive.Store) {
	...
	if err != nil {
		log.Printf("resolve configuration path: %v", err)
		return
	}
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			if filepath.Clean(event.Name) != absolute || event.Op&(fsnotify.Write|fsnotify.Create|fsnotify.Rename|fsnotify.Remove) == 0 {
				continue
			}
			if timer != nil {
				timer.Stop()
		...
		case <-reload:
			...
			if err != nil {
				log.Printf("configuration reload rejected (%T); retaining previous configuration", err)
				continue
			}
			current := state.Config()
			if conf.Port != current.Port {
				log.Print("port change requires a restart; retaining current port")
				conf.Port = current.Port
			}
...
func Check(ctx context.Context, state *appstate.State, archiveStore *archive.Store, _ string, result *gofeed.Feed, item *gofeed.Item) {
	if ctx.Err() != nil || result == nil || item == nil || len(result.Items) == 0 {
		return
	}
	link := normalizeLink(item.Link)
	if link == "" || archiveStore.Contains(link) {
		return
	}
	conf := state.Config()
