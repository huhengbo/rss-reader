...
func main() {
	...
	reset := func() {
		...
		for index, title := range []string{"技术周刊", "同站专题", "空订阅"} {
			...
			if index < 2 {
				for i := 0; i < 12; i++ {
					feed.Items = append(feed.Items, domain.Item{GUID: fmt.Sprintf("item-%d", i), Title: fmt.Sprintf("%s：第 %d 篇阅读与开发笔记", title, i+1), Link: fmt.Sprintf("https://example.com/article?id=%d", i+index*100), PublishedAt: "2026-09-12T00:00:00Z"})
				}
			}
	...
	mux.HandleFunc("POST /fixture/{action}", func(w http.ResponseWriter, r *http.Request) {
		...
		case "reset":
			reset()
		case "new":
			f, _ := state.Feed(urls[0])
			f.Items = append(f.Items, domain.Item{GUID: "fresh", Title: "新文章：可靠的实时阅读", Link: "https://example.com/fresh"})
			state.SetFeed(urls[0], f)
		case "delete":
			c := state.Config()
			c.Values = urls[1:]
			state.ReplaceConfig(c)
		case "empty":
			c := state.Config()
			c.Values = nil
			state.ReplaceConfig(c)
		case "zero":
			...
			state.ReplaceConfig(c)
		case "fail":
			ticket, _ := state.BeginFetch(urls[0], time.Now())
			state.CompleteFetch(urls[0], ticket, domain.Feed{}, time.Now(), errors.New("do not disclose this upstream URL"))
		case "long":
			f, _ := state.Feed(urls[0])
			f.Title = strings.Repeat("长标题", 20)
			f.Items[0].Title = strings.Repeat("LongUnbrokenTitle", 30)
			state.SetFeed(urls[0], f)
		default:
			http.NotFound(w, r)
			return
